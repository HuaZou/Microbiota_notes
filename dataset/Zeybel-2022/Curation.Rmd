...
get_metaphlan_phyloseq <- function(otu_tab,
  ...
  sid <- intersect(sam_tab$`Patient ID`, colnames(otu_tab))
  
  sam_tab <- sam_tab[sam_tab$`Patient ID` %in% sid, ] %>%
    tibble::column_to_rownames("Patient ID")
  otu_tab <- otu_tab %>%
...
```{r}
metabolites_fecal$metabolitesID <- paste0("Chem_", metabolites_fecal$`CHEMICAL ID`)
...
metabolites_fecal_colData <- metadata  %>%
  dplyr::filter(`Patient ID` %in% all_of(colnames(metabolites_fecal_assay)))
...
protein_plasma_colData <- metadata %>%
  dplyr::filter(`Patient ID` %in% all_of(colnames(protein_plasma_assay)))
...
metadata_before <- metadata %>%
  dplyr::filter(`Patient ID` %in% metadata_after$`Patient ID`) %>%
  dplyr::mutate(Stage = "Before")

# cbind
long_metadata <- rbind(metadata_before, metadata_after) %>%
  dplyr::mutate(SampleID = paste(`Patient ID`, Stage, sep = "_")) %>%
  dplyr::select(SampleID, `Patient ID`, Gender, Age, Stage, 
                `Liver Fat Class`, 
                `Weight (kg)`, `Body mass index (kg/m2)`, 
                `Hip circumference (cm)`, 
                `Waist circumference (cm)`, everything()) %>%
  dplyr::arrange(`Patient ID`, desc(Stage))
...
+ SummarizedExperiment: metabolites
```{r}
metabolites_ID <- intersect(metabolites_fecal$BIOCHEMICAL, long_metabolites_fecal$`Metabolite ID`)
...
long_metabolites_fecal_assay_after <- long_metabolites_fecal %>%
  dplyr::inner_join(long_metabolites_fecal_rowData %>% 
                    ...
                    by = c(`Metabolite ID` = "BIOCHEMICAL")) %>%
  dplyr::select(-`Metabolite ID`) %>%
  dplyr::select(c("metabolitesID", c(starts_with("P10")))) %>%
  tibble::column_to_rownames("metabolitesID")
...
+ SummarizedExperiment: proteomics
```{r}
long_protein_plasma$`Protein ID` <- gsub("-", "_", long_protein_plasma$`Protein ID`)
proteins_ID <- intersect(protein_plasma_new$ProteinID, long_protein_plasma$`Protein ID`)


# Row->proteins; Col->Samples
long_protein_plasma_assay_after <- long_protein_plasma %>%
  dplyr::filter(`Protein ID` %in% proteins_ID) %>%
  dplyr::select(c(`Protein ID`, c(starts_with("P10")))) %>%
